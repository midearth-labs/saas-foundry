...
export class SubscriptionWaitlistClient implements SubscriptionWaitlistClientInterface {
  ...
  private subscription!: ActiveSubscriptionShape;
  ...
  public createUsers() {
    console.log("\n1. Creating users...");
    return createEmailUserOrThrow(this.Users.Regular.name, this.Users.Regular.email, this.Users.Regular.password)
      .then(() => createEmailUserOrThrow(this.Users.Subscriber.name, this.Users.Subscriber.email, this.Users.Subscriber.password))
      .then(() => {
  ...
  public setupSubscription() {
    ...
    .then(response => {
      ...
      if (activeSubscription) {
        ...
        const placeholderSubscription: ActiveSubscriptionShape = {
          plan: PRO_PLAN,
          status: "active",
          id: this.users.subscriber!.id,
          limits: undefined,
          referenceId: this.users.subscriber?.id ?? '',
          periodEnd: undefined,
