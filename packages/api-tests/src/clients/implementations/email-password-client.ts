...
export class EmailPasswordClient implements AuthClientInterface {
  ...
  public createUser(name: string, email: string, password: string) {
    ...
    return createEmailUserOrThrow(name, email, password);
  }
  ...
  public signInUser(email: string, password: string) {
    ...
    return signInUserOrThrow(email, password)
      .then(({ signedInUser }) => signedInUser);
  }
   ...
   */
  public signInUnsuccessfully(email: string, password: string) {
    console.log(`\nAttempting unsuccessful sign in with ${email}...`);
    return signInUnsuccessfully(email, password + "BAD")
      .then(({ unsuccessfulUser }) => unsuccessfulUser);
  }
   ...
   */
  public execute() {
    const execution = this.createUser(this.USER_NAME, this.USER_EMAIL, this.USER_PASSWORD)
      .then(() => {
      ...
      .then((signedInUser) => {
        ...
        return this.signInUnsuccessfully(this.USER_EMAIL, this.USER_PASSWORD)
          .then((unsuccessfulUser) => {
            console.log("\nUnsuccessful sign-in caught as expected");
