...
import { getAuthClient, getUserInput, AuthClientType, CreateEmailUserResult } from '../utils';
...
export class LinkAccountClient implements LinkAccountClientInterface {
    private readonly googleAuthClient: AuthClientType;
    private readonly emailAuthClient: AuthClientType;
    private userContext: {
        email: string;
        name: string;
        password: string;
     ...
     */
    public async createExtendedEmailUser(shouldSignIn?: boolean) {
        ...
        .then(async (user) => {
            if (user && !user.error) {
                if (shouldSignIn) {
                    const signedInUser = await this.signInExtendedEmailUser(false);
     ...
     */
    public async signInExtendedEmailUser(shouldSignOut?: boolean) {
        ...
        if (signInResponse && !signInResponse.error) {
            ...
            if (shouldSignOut) {
                console.log("\nSafely signing out...");
                return this.signOutExtendedUser(this.emailAuthClient);
            }
     ...
     */
    public async signInExtendedGoogleUser(shouldSignOut?: boolean) {
                ...
                .then(async () => {
                    try {
                        if (shouldSignOut) {
                            ...
                            await this.signOutExtendedUser(this.googleAuthClient);
                        }
     ...
     */
    public async signOutExtendedUser(client: AuthClientType) {
        try {
     ...
     */
    public async updateUserProfile(client: AuthClientType, title?: string) {
        ...
        if (linkedAccount && !linkedAccount.error) {
            ...
            return new Promise(async (resolve) => {
                try {
                    ...
                    await this.signOutExtendedUser(this.emailAuthClient);
                    console.log("\nSigning in again via Google to verify profile updates...");
                    await this.signInExtendedGoogleUser();
    ...
    public execute(): Promise<JSON> {
        ...
        const execution = this.createExtendedEmailUser(true) // Pass true to sign in after creation
            .then(() => {
            ...
            .then(() => {
                ...
                return this.signInExtendedGoogleUser(true); // Pass true to sign out after Google sign-in
            })
            .then(() => {
                ...
                return this.signInExtendedEmailUser(false); // Pass false to maintain session
            })
            .then(() => {
            ...
            .then(() => {
                ...
                return this.signInExtendedGoogleUser(false); // Don't sign out after Google sign-in
            })
            .then(() => {
