...
export const createDrizzleWaitListEntryRepository = (db: NodePgDatabase<any>): WaitListEntryRepository => {
  ...
  return {
    async create(data: wl.CreateWaitListEntryDto): Promise<wl.WaitListEntryIdDto> {
      ...
      .from(waitlistDefinitions)
      .where(and(
        eq(waitlistDefinitions.id, data.definitionId),
        eq(waitlistDefinitions.tier, 'BASIC'),
        eq(waitlistDefinitions.status, 'ACTIVE')
      ));
    
      if (definition.length === 0) {
        throw new Error('No active BASIC tier waitlist found with the provided definition ID');
      }
    ...
    async createProEntry(data: wl.CreateProWaitListEntryDto): Promise<wl.WaitListEntryIdDto> {
      // First validate that the definition is PRO tier
      const definition = await db.select()
        .from(waitlistDefinitions)
        .where(and(
          eq(waitlistDefinitions.id, data.definitionId),
          eq(waitlistDefinitions.tier, 'PRO'),
          eq(waitlistDefinitions.status, 'ACTIVE')
        ));
      
      if (definition.length === 0) {
        throw new Error('No active PRO tier waitlist found with the provided definition ID');
      }
    ...
    async createStandardEntry(data: wl.CreateStandardWaitListEntryDto): Promise<wl.WaitListEntryIdDto> {
      // First validate that the definition is STANDARD tier
      const definition = await db.select()
        .from(waitlistDefinitions)
        .where(and(
          eq(waitlistDefinitions.id, data.definitionId),
          eq(waitlistDefinitions.tier, 'STANDARD'),
          eq(waitlistDefinitions.status, 'ACTIVE')
        ));
      
      if (definition.length === 0) {
        throw new Error('No active STANDARD tier waitlist found with the provided definition ID');
      }
